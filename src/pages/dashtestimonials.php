...
<footer>
  ...
  const fetchProjects = async () => {
    try {
      const response = await fetch("http://localhost/backend/testimonials.php", {
      method: "GET",
    });
    if (response.ok) {
...
const deleteProject = async (id) => {
  try {
    const response = await fetch(`http://localhost/backend/testimonials.php?id=${id}`, {
      method: "DELETE",
    });
...
const initializeTable = (data) => {
    ...
    columns: [
      ...
      { data: "created_at" },
      {
        data: null,
        defaultContent:
          '<div class="flex gap-2"><button type="button" class="focus:outline-none text-white bg-[#FE8D4D] hover:bg-[#F18040] focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-[#F18040] deleteButton">Delete</button></div>',
      },
    ],
  });
