...
switch ($method) {
    case "GET":
        $sql = "SELECT `categoryID`, `categoryName` FROM `sys3`.`categories`"; 
        ...
        if ($stmt) {
            if ($stmt->execute()) {
                ...
                echo json_encode(['status' => 'success', 'message' => 'categories fetched successfully', 'content' => $projects]);
            } else {
    ...
    case "POST": 
        ...
        $sql = "INSERT INTO `sys3`.`categories`(categoryName) VALUE (?)";
        $stmt = $mysqli->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("s",$data->categoryName);
            if ($stmt->execute()) {
