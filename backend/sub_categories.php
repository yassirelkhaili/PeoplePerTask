...
switch ($method) {
    case "GET":
        $sql = "SELECT `sub_categoryID`, `sub_categoryName`, `categoryID` FROM `sys3`.`sub_categories`"; 
        ...
        if ($stmt) {
            if ($stmt->execute()) {
                ...
                echo json_encode(['status' => 'success', 'message' => 'Sub_categories fetched successfully', 'content' => $projects]);
            } else {
    ...
    case "POST": 
        ...
        $sql = "INSERT INTO `sys3`.`sub_categories`(sub_categoryName, categoryID) VALUE (?, ?)";
        $stmt = $mysqli->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("si",$data->sub_categoryName, $data->categoryID);
            if ($stmt->execute()) {
                http_response_code(201);
                echo json_encode(['status' => 'success', 'message' => 'Sub_category created successfully']);
            } else {
                http_response_code(400);
                echo json_encode(['status'=> 'error','message'=> 'An error was encountered' . $stmt->error]);
            }
