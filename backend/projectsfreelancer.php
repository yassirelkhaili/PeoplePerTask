...
switch ($method) {
    case "GET":
        $sql = "SELECT `projects`.`projectID`, `projects`.`ProjectTitle`, `projects`.`projectDesc`, `categories`.`categoryName`, `sub_categories`.`sub_categoryName`, `users`.`userID` FROM `sys3`.`projects` JOIN `categories` ON `projects`.`CategoryID` = `categories`.`CategoryID` JOIN `sub_categories` ON `projects`.`sub_categoryID` = `sub_categories`.`sub_categoryID` JOIN `users` ON `projects`.`UserID` = `users`.`UserID` where `users`.`userID` = $_SESSION[userID]";
